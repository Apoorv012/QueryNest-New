...
# first; uvicorn binds IPv4-only by default, so every request stalls ~2s on
# the failed IPv6 attempt before falling back. Measured: 2083ms via
# "localhost" vs 16ms via 127.0.0.1 for an identical request — which made
# every reported search latency a measurement artifact rather than a
# property of the search path (real cost: ~0.04ms parse, ~5ms embed,
# ~8ms pgvector).
...
def get_document_filename_map(user_id: str = EVAL_USER_ID) -> dict[str, str]:
    ...
    base = os.environ.get("QUERYNEST_API_URL", DEFAULT_API_URL)
...
def score_retrieved(
    ...
    """Turn a ranked list of raw retrieved document_ids into a scored QueryResult.

    Shared by every retriever evaluated against the golden set (semantic in
    `run_eval` below, BM25 in `core/eval/baselines.py`) so the dedup and
    metric logic — the part that was actually buggy historically (see
    docs/plan.md 1.1-1.3) — exists in exactly one place, and results from
    different retrievers stay directly comparable.
    """
...
def run_eval(
    ...
    for q in queries:
        ...
        results.append(score_retrieved(q, retrieved_doc_ids_raw, id_to_filename, latency_ms))
